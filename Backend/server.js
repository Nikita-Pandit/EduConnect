...
//     student.selectStudent.set(teacherID, true);
//     await student.save();

//     res.status(200).json({ message: "Student selected successfully", student });
//   } catch (error) {
//     console.error("Error selecting students:", error);
//     res.status(500).json({ message: "Internal Server Error", error });
//   }
...
//   try {
//     const students = await studentMoreInfo.find(
//       { [`selectStudent.${teacherID}`]: true },
//       "name rollNo domain branch image"
//     );
//     if (students.length > 0) res.json({ students });
//     else res.status(404).json({ message: "No students under supervision" });
//   } catch (error) {
//     console.error("Error fetching supervised students:", error);
//     res.status(500).json({ message: "Error fetching data" });
//   }
...
// // Get teachers who selected a student
// app.get("/api/selectedTeachers/:studentId", async (req, res) => {
//   try {
//     const { studentId } = req.params;
//     const student = await studentMoreInfo.findOne({ studentID: studentId });
